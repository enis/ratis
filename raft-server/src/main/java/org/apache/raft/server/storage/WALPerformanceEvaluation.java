...
import org.apache.hadoop.util.Tool;package org.apache.hadoop.hbase.wal;
...
/**
 * Adapted from HBase's WALPerformanceEvaluation
 */
public final class WALPerformanceEvaluation extends Configured implements Tool {
  ...
  class WALPutBenchmark implements Runnable {
    ...
    WALPutBenchmark(final RaftLog log, final long numIterations, final boolean noSync,
      ...
      try {
        ...
        for (int i = 0; i < numIterations; ++i) {
          try {
            ...
            Message m = createMessage(rand, key, value);
            LogEntryProto proto = LogEntryProto.newBuilder()
                .build();
  ...
  private Message createMessage(Random random, byte[] key, byte[] value) {
    return null;
  }
    ...
    try {
      ...
      try {
        for(int i = 0; i < numRegions; i++) {
          ...
          benchmarks[i] = new WALPutBenchmark(regions[i], htd, numIterations, noSync,
  ...
  public static void main(String[] args) throws Exception {
    System.exit(innerMain(new Configuration(), args));
  }
}
